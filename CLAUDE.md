...
- **Fitness** (`fitness.ts`): 3-fold temporal cross-validation (not single train/test split). Fitness = 35% OOS PPG + 35% trust-scaled live PPG + 12% overfit penalty (gap×3.0) + 8% fold consistency penalty (penalizes high variance across CV folds) + 10% risk-adjusted return + 5% diversity bonus. Live PPG only counts genuine pre-committed predictions (source='prediction'), not replays. Trust scaling: sqrt(n/50) clamped to [0,1]
- **Evolution** (`evolve.ts`): 20 strategies per spot count (1-10), 10 survive. SUS parent selection, 40% BLX-alpha crossover (blends numeric genes between parents with α=0.3 exploration) / 60% mutation, 1 elite carry-forward + 2 random immigrants per generation. Mutation rate: 50%->35%->20%->12% (never below 12%). Champions promoted per spot count if they beat the incumbent
